...
void OLED_DisplayGIF(const unsigned char gif[][1024]) {
    ...
    if (cur_frame >= 15) {
        cur_frame = 15;
        frames_period = dwt->SysTime.ms_tick;
        OLED_DisplayBMG(gif[cur_frame]);
        frames_period++;
        cur_frame = 0;
    }
    else if (((uint32_t)(dwt->SysTime.ms_tick) - frames_period) >= frame) {
        frames_period = (uint32_t)(dwt->SysTime.ms_tick);
        OLED_DisplayBMG(gif[cur_frame]);
        cur_frame++;
    }
}
